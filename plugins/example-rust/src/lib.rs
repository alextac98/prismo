...
use prismo_core::{
    ...
    PluginStatusUpdate, TelemetryUpdate,
};
use rand::{Rng, SeedableRng, rngs::StdRng};

pub const EXAMPLE_PLUGIN_ID: &str = "example-rust";
...
impl SyntheticChannelSpec {
    ...
    fn sample(
        ...
        ChannelSample {
            ...
            value,
            observed_at,
            source_timestamp_unix_ns: timestamp_unix_ns,
            sequence,
        }
    }
}

pub struct ExampleTelemetrySource {
    period: Duration,
    specs: Vec<SyntheticChannelSpec>,
    rng: StdRng,
    tick: u64,
    emitted_updates: u64,
    dropped_updates: u64,
    base_timestamp_unix_ns: u64,
}

impl ExampleTelemetrySource {
    pub fn new(period: Duration) -> Self {
        Self::from_seed(period, rand::rng().random())
    }

    pub fn from_seed(period: Duration, seed: u64) -> Self {
        Self {
            period,
            specs: specs(),
            ...
            emitted_updates: 0,
            dropped_updates: 0,
            base_timestamp_unix_ns: 1_720_000_000_000_000_000,
        }
    }
    ...
    pub fn initial_status(&self) -> PluginStatusUpdate {
        PluginStatusUpdate {
    ...
    pub fn next_update(&mut self, observed_at: Instant) -> TelemetryUpdate {
        self.tick += 1;
        self.emitted_updates += 1;

        let timestamp_unix_ns =
            self.base_timestamp_unix_ns + self.tick * self.period.as_nanos() as u64;
        let samples = self
            .specs
            .iter()
            .copied()
            .filter(|spec| spec.should_emit(self.tick))
            .map(|spec| {
                spec.sample(
                    &mut self.rng,
                    self.tick,
        ...
        self.dropped_updates += self
            .specs
            .iter()
            .copied()
            .filter(|spec| spec.is_dropped_out(self.tick) && self.tick % spec.every_ticks == 0)
            .count() as u64;

        let current_outages = self
            .specs
            .iter()
            .copied()
            .filter(|spec| spec.is_dropped_out(self.tick))
            .map(|spec| spec.path)
            .take(3)
            .collect::<Vec<_>>();
...
impl Default for ExampleTelemetrySource {
    fn default() -> Self {
        Self::new(Duration::from_millis(200))
    }
}

fn specs() -> Vec<SyntheticChannelSpec> {
